        ...
        <div class="grid gap-x-4 md:grid-cols-2 lg:grid-cols-3">
            <x-card title="System" class="p-0 shadow-none" style="padding: 0 !important;">
                ...
                <x-label title="Credits Display Name">
                    <x-input x-todo="credits-display-name" id="credits-display-name" name="credits-display-name"
                ...
                <x-label title="Pterodactyl API Key">
                    <x-input x-todo="pterodactyl-api-key" id="pterodactyl-api-key" name="pterodactyl-api-key"
                        type="text" value="{{ config('SETTINGS::SYSTEM:PTERODACTYL:TOKEN') }}" reauired />
                    @slot('text')
                        {{ __('Enter the API Key to your Pterodactyl installation.') }}
                    @endslot
                </x-label>
                <x-label title="Pterodactyl Admin-Account API Key">
                    <x-input x-todo="pterodactyl-admin-api-key" id="pterodactyl-admin-api-key"
                        name="pterodactyl-admin-api-key" type="text"
                        value="{{ config('SETTINGS::SYSTEM:PTERODACTYL:ADMIN_USER_TOKEN') }}" reauired />
                    @slot('text')
            ...
            <x-card title="User" class="p-0 shadow-none" style="padding: 0 !important;">
                <x-checkbox title="Force Discord Verification" value="true" id="force-discord-verification"
                    name="force-discord-verification"
                    checked="{{ config('SETTINGS::USER:FORCE_DISCORD_VERIFICATION') }}">
                </x-checkbox>
                ...
                <x-label title="Initial Server Limit">
                    <x-input x-todo="initial-server-limit" id="initial-server-limit" name="initial-server-limit"
                        type="number" value="{{ config('SETTINGS::USER:INITIAL_SERVER_LIMIT') }}" required />
                </x-label>
                ...
                <x-label title="Server Limit Increase - Discord">
                    <x-input x-todo="server-limit-discord" id="server-limit-discord" name="server-limit-discord"
                        type="number"
                        value="{{ config('SETTINGS::USER:SERVER_LIMIT_REWARD_AFTER_VERIFY_DISCORD') }}" required />
                </x-label>
                <x-label title="Server Limit Increase - Email">
                    <x-input x-todo="server-limit-email" id="server-limit-email" name="server-limit-email"
                        type="number" value="{{ config('SETTINGS::USER:SERVER_LIMIT_REWARD_AFTER_VERIFY_EMAIL') }}"
                        required />
                </x-label>
                ...
                <x-label title="Server Limit after Credits Purchase">
                    <x-input x-todo="server-limit-purchase" id="server-limit-purchase" name="server-limit-purchase"
                        type="number" value="{{ config('SETTINGS::USER:SERVER_LIMIT_AFTER_IRL_PURCHASE') }}"
                        requried />
                </x-label>
            </x-card>
            ...
            <x-card title="Other" class="p-0 shadow-none" style="padding: 0 !important;">
                <x-card title="Server" class="p-0 shadow-none" style="padding: 0 !important;">
                    <x-checkbox title="Creation or new Servers" value="true" id="enable-disable-servers"
                        ...
                        checked="{{ config('SETTINGS::SYSTEM:CREATION_OF_NEW_SERVERS') }}">
                    ...
                    <x-label title="Server Allocation Limit">
                        <x-input x-todo="allocation-limit" id="allocation-limit" name="allocation-limit"
                            type="number" value="{{ config('SETTINGS::SERVER:ALLOCATION_LIMIT') }}" required />
                        @slot('text')
                ...
                <x-card title="Design" class="p-0 shadow-none" style="padding: 0 !important;">
                    ...
                    <x-checkbox title="Show logo on Login Page" value="true" id="enable-login-logo"
                        name="enable-login-logo" checked="{{ config('SETTINGS::SYSTEM:ENABLE_LOGIN_LOGO') }}">
                    </x-checkbox>

                    <x-label title="Panel Icon">
                        <x-input type='file' accept="image/png,image/jpeg,image/jpg" class="custom-file-input"
                            name="icon" id="icon"></x-input>
                    </x-label>
                    <x-label title="Login Page Icon">
                        <x-input type='file' accept="image/png,image/jpeg,image/jpg" class="custom-file-input"
                            name="logo" id="logo"></x-input>
                    </x-label>
