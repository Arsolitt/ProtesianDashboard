        ...
        <div class="ease-soft-in-out xl:ml-68.5 relative transition-all duration-200 text-gray-700 dark:text-gray-200">
            ...
            @if (is_null(Auth::user()->discordUser) && strtolower($force_discord_verification) == 'true')
                @if (!empty(config('SETTINGS::DISCORD:CLIENT_ID')) && !empty(config('SETTINGS::DISCORD:CLIENT_SECRET')))
                    <x-alert class="mt-6" type="danger" title="Требуется привязка Discord">
                        <p>
                            {{ __('Вы еще не подтвердили свой Discord') }} <br>
                            <a class="text-primary underline"
                               href="{{ route('auth.redirect') }}">{{ __('Подтвердить Discord') }}</a>
                            <br>
                            {{ __('Если у вас возникнут какие-либо проблемы, обратитесь в службу поддержки.') }}
                        </p>
                    </x-alert>
                @else
                    <x-alert class="mt-6" type="danger" title="Required Discord verification">
                        <p>
                            {{ __('В связи с системными настройками вам необходимо подтвердить свой аккаунт в Discord!') }}
                            <br>
                            {{ __('Похоже, что он не был настроен правильно! Пожалуйста, обратитесь в службу поддержки.') }}
                        </p>
                    </x-alert>
                @endif
            @endif
                    ...
                    <div class="flex flex-wrap -mx-3">
                        <div class="flex-none w-auto max-w-full px-3">
                            <div
                                ...
                                <img src="{{ $user->getAvatar() }}" alt="profile_image"
                ...
                <div class="grid gap-6 mb-8 md:grid-cols-2">
                    <div
                        ...
                        <div class="p-4 pb-0 mb-0 bg-white dark:bg-gray-800 border-b-0 rounded-t-2xl">
                            <div class="flex flex-wrap -mx-3">
                                <div class="flex items-center w-full max-w-full px-3 shrink-0 md:w-8/12 md:flex-none">
                                    <h2 class="font-bold">{{ __('Информация профиля') }}</h2>
                        ...
                        <div class="p-4 pb-0">
                            <ul class="flex flex-col pl-0 mb-0 rounded-lg">
                                <li
                                    ...
                                    <strong class="text-gray-700 dark:text-gray-200">Ник:</strong> &nbsp;
                                    {{ $user->name }}
                                </li>
                                <li
                                    class="relative block px-4 py-2 pl-0 leading-normal bg-white dark:bg-gray-800 border-0 border-t-0 text-sm text-inherit">
                                    <strong class="text-gray-700 dark:text-gray-200">Почта:</strong> &nbsp;
                                    {{ $user->email }}
                                </li>
                                @if (config('SETTINGS::REFERRAL::ENABLED') == 'true')
                                    <li data-content="Click to Copy" data-toggle="popover" data-trigger="hover"
                                        data-placement="top"
                                        ...
                                        @else
                                            <span
                                                class="px-2 py-1 text-xs font-semibold leading-tight rounded-full badge-warning">
                        {{ _('Make a purchase to reveal your referral-URL') }}</span>
                                ...
                                <li
                                    class="relative block px-4 py-2 pl-0 leading-normal bg-white dark:bg-gray-800 border-0 border-t-0 text-sm text-inherit">
                                    <strong class="text-gray-700 dark:text-gray-200">Статус:</strong> &nbsp; <span
                                        class="px-2 py-1 text-xs font-semibold leading-tight rounded-full {{ $badgeColor }}">{{ __($user->role) }}</span>
                                </li>
                                <li
                                    class="relative block px-4 py-2 pt-0 pl-0 leading-normal bg-white dark:bg-gray-800 border-0 rounded-t-lg text-sm text-inherit">
                                    <strong class="text-gray-700 dark:text-gray-200">Создан:</strong> &nbsp;
                                    {{ $user->created_at->isoFormat('LL') }}
                                </li>
                            </ul>
                        </div>

                        <x-button class="mx-4 my-2 bg-red-500 hover:bg-red-600 text-white w-1/3"
                                  id="confirmDeleteButton"
                                  type="button">{{ __('Удалить аккаунт') }}</x-button>
                        @if (!empty(config('SETTINGS::DISCORD:CLIENT_ID')) && !empty(config('SETTINGS::DISCORD:CLIENT_SECRET')))
                            <div class="p-4 pt-0 mt-4">
                                ...
                                @else
                                    ...
                                    <div
                                        class="flex items-center space-x-4 p-4 rounded-lg dark:bg-gray-700 bg-gray-100">
                                        ...
                                        <div class="e-profile">
                                            ...
                                            <div class="tab-content">
                                                ...
                                                <label class="block text-sm mb-3">
                                                    <span
                                                        class="text-gray-700 dark:text-gray-400">{{ __('Имя') }}</span>
                                                ...
                                                <label class="block text-sm mb-3">
                                                    <span
                                                        class="text-gray-700 dark:text-gray-400">{{ __('Почта') }}</span>
                                                        ...
                                                        <div class="row">
                                                            <div class="col">
                                                                <div class="form-group">
                                                                    <label class="block text-sm mb-3">
                                                                        <span
                                                                            class="text-gray-700 dark:text-gray-400">{{ __('Текущий пароль') }}</span>
                                                                        <input x-todo="current_password"
                                                                               id="current_password"
                                                        ...
                                                        <div class="row">
                                                            <div class="col">
                                                                <div class="form-group">
                                                                    <label class="block text-sm mb-3">
                                                                        <span
                                                                            class="text-gray-700 dark:text-gray-400">{{ __('New Password') }}</span>
                                                                        <input x-todo="new_password" id="new_password"
                                                        ...
                                                        <div class="row">
                                                            <div class="col">
                                                                <div class="form-group">
                                                                    ...
                                                                    <label class="block text-sm mb-3">
                                                                        <span
                                                                            class="text-gray-700 dark:text-gray-400">{{ __('Confirm Password') }}</span>
                                                                        <input x-todo="new_password_confirmation"
                                                ...
                                                <div class="row">
                                                    <div class="col d-flex justify-content-end">
                                                        ...
                                                        <button
                                                            ...
                                                            type="submit">{{ __('Сохранить изменения') }}</button>
                                                    </div>
                                                </div>
    ...
    <script>
        document.getElementById("confirmDeleteButton").onclick = async () => {
            const {
                ...
                background: "#343a40",
                input: "text",
                inputLabel: '{{ __('Вы уверены, что хотите навсегда удалить свою учетную запись и все свои серверы?') }} \n Напишите "{{ __('Удалить') }}" в строке ниже',
                inputPlaceholder: "{{ __('Удалить') }}",
                showCancelButton: true
            });
            if (enterConfirm === "{{ __('Удалить') }}") {
                Swal.fire("{{ __('Аккаунт был уничтожен') }}", "", "error");
                $.ajax({
                    type: "POST",
                ...
                Swal.fire("{{ __('Аккаунт НЕ был удалён.') }}", "", "info");
