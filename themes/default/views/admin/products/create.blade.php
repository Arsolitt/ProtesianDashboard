        ...
        <form action="{{ route('admin.products.store') }}" method="POST">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <x-card title="Details">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        <x-label title="Name">
                            <x-input value="{{ $product->name ?? old('name') }}" id="name" name="name"
                                type="text" />
                        </x-label>
                        <x-label title="Price in {{ CREDITS_DISPLAY_NAME }}">
                            <x-input value="{{ $product->price ?? old('price') }}" id="price" name="price"
                                step=".01" type="number" />
                        </x-label>
                        <x-label title="Memory">
                            <x-input value="{{ $product->memory ?? old('memory') }}" id="memory" name="memory"
                                type="number" />
                        </x-label>
                        <x-label title="CPU">
                            <x-input value="{{ $product->cpu ?? old('cpu') }}" id="cpu" name="cpu"
                                type="number" />
                        </x-label>
                        <x-label title="Disk">
                        ...
                        <x-label title="Swap">
                            <x-input value="{{ $product->swap ?? old('swap') }}" id="swap" name="swap"
                                type="number" />
                        </x-label>
                        ...
                        <x-label title="Minimum {{ CREDITS_DISPLAY_NAME }}">
                            <x-input value="{{ $product->minimum_credits ?? (old('minimum_credits') ?? -1) }}"
                                id="minimum_credits" name="minimum_credits" type="number" />
                        </x-label>
                        <x-label title="IO">
                            <x-input value="{{ $product->io ?? (old('io') ?? 500) }}" id="io" name="io"
                                type="number" />
                        </x-label>
                        <x-label title="Databases">
                            <x-input value="{{ $product->databases ?? (old('databases') ?? 1) }}" id="databases"
