...
@section('content')
    <x-container title="Products">
        ...
        @if ($product->servers()->count() > 0)
            <x-alert
                title="Editing the resource options will not automatically update the servers on
        pterodactyls side!"
                type="danger">
                ...
                <x-card title="{{ __('Product Details') }}">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <x-label title="Price in {{ CREDITS_DISPLAY_NAME }}">
                            <x-input value="{{ $product->price }}" id="price" name="price" step=".01"
                                type="number" />
                        </x-label>
                        ...
                        <x-label title="Allocations">
                            <x-input value="{{ $product->allocations }}" id="allocations" name="allocations"
                                type="number" />
                        </x-label>
                    </div>
                                ...
                                @foreach ($locations as $location)
                                    <optgroup label="{{ $location->name }}">
                                        @foreach ($location->nodes as $node)
                                            <option @if (isset($product) && $product->nodes->contains('id', $node->id)) selected @endif
                                ...
                                @foreach ($nests as $nest)
                                    <optgroup label="{{ $nest->name }}">
                                        @foreach ($nest->eggs as $egg)
                                            <option @if (isset($product) && $product->eggs->contains('id', $egg->id)) selected @endif
