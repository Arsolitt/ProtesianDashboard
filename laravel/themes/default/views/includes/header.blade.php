<header class="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
    <div class="container flex items-center justify-between h-full px-6 mx-auto text-purple-600 dark:text-purple-300">
        ...
        <svg class="w-4 h-4 " fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25"></path>
</svg>
                <a href="{{ $link->link }}" class="" target="__blank"><i
                        class="{{$link->icon}}"></i> {{ __($link->title) }}</a>
        </div>
        @endforeach
        </div>
        <!-- Search input -->
        ...
        <ul class="flex items-center flex-shrink-0 space-x-6">
            ...
            @if (config('SETTINGS::LOCALE:CLIENTS_CAN_CHANGE') == 'true')
                <li class="relative">
                    <button class="relative align-middle rounded-md focus:outline-none focus:shadow-outline-purple"
                        @click="toggleLanguageMenu" @keydown.escape="closeLanguageMenu" aria-label="Language"
                        aria-haspopup="true">
                    ...
                    <ul x-cloak x-show="isLanguageMenuOpen" x-transition:leave="transition ease-in duration-150"
                        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                        @click.away="closeLanguageMenu" @keydown.escape="closeLanguageMenu"
            ...
            @endif
            <p class="hidden sm:inline-flex sm:items-center px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200">
            ...
            <li class="relative">
                ...
                <ul x-cloak x-show="isProfileMenuOpen" x-transition:leave="transition ease-in duration-150"
                    ...
                    aria-label="submenu">
                    <li class="flex">
                        <p class="inline-flex items-center sm:hidden w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200">
                            <span>{{ __('Balance') }}: {{ Auth::user()->credits() }}</span>
                            <svg class="w-4 h-4 ml-1" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                      d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z"
